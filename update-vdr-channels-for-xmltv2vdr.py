...
class ChannelHandler( xml.sax.ContentHandler ):
  ...
  def characters(self, content):
    if self.CurrentData == "display-name":
      chanIds = list()
      if ChannelIds.has_key(self.ChannelId):
        chanIds = ChannelIds[self.ChannelId]
      chanIds.append(content.lower())
      ChannelIds[self.ChannelId] = chanIds

def process(src, dst):

  for ligne in src:
    donnees = ligne.rstrip('\n\r').split(":")
    channelName = (donnees[0].split(";")[0]).lower()
    results = list()
    for chan in ChannelIds:
      ok = 0
      for chanName in chan[1]:
    ...
    if len(results) == 1 :
      ...
      print channelName + ' plusieurs correspondance' + '->' + results.__str__()
    elif len(results) == 0:

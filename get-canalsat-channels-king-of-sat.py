...
class app_params:
    ...
    sat_list            = {'19.2E': 'http://fr.kingofsat.net/tv-19.2E.php'}

    sat_id              = {'19.2E': '19.2'}

    caid_bouquets       = {'CanalSat'           : '100,500,1811' }
...
class getchannels:
    ...
    def __parse_args(self):
        ...
        parser.add_argument('-o', '--output', type=str, metavar='FILENAME', dest='outfile', required=False,
                            help='enable use of config file, searched in "conf" subfolder, to generate output')
    ...
    # write output std (channels list)
    def __write_output_std(self):
        outputStr = ""
        outputStrMissing = ""
        ...
        if self.__args.configfile != None:
            ...
            # update output string with all data or one bouquet
            for item in range(0, len(self.__name_bouquets)):
                if self.__args.bouquet != None:
                    #filter by bouquet
                    if self.__name_bouquets[item] == self.__args.bouquet:
                        outputStr += self.__channels_bouquets[item]
                else:
                    outputStr += ":[ " + self.__name_bouquets[item] + " ]\n" + self.__channels_bouquets[item]


        # write data in to output std
        print outputStr

        # check for missing channels
        if (len(outputStrMissing) > 0):
            # write data in missing output std
            print "missing"
            print outputStrMissing
